  ...
  <div class="dashboard-container">
    <div class="container">
      <div class="tableBar">
        ...
        <div class="tableLab">
          <span class="delBut non" @click="deleteHandle('批量')">批量删除</span>
...
export default class extends Vue {
  ...
  private addSetMeal(st: any) {
    if (st === 'add') {
      this.$router.push({ path: '/admin/setmeal/add' })
    } else {
      // 相当于拼接url成'/setmeal/add?id=${st.id}'
      this.$router.push({ path: '/admin/setmeal/add', query: { id: st.id } })
    }
  }
      ...
      .then(res => {
        if (res && res.data && res.data.code === 1) {
          this.dishCategoryList = (
            res.data &&
            res.data.data &&
            // 这里最后一个元素,即res.data.data用于下面的map
            res.data.data
          ).map(item => {
